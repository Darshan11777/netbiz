$(document).ready(function() {
	...
	//VOUCHER REDIRECT	
	var exclude_nl = ['2829','2865','2823','2831','2812','2574','2887','2813','2896','2745','2818','2660','2960','2953','2964','2950','2811','3011','3018','3071','3034','3060'];
		...
		if( $('#telnr-bevestiging').length > 0 ){
			...
			redirect = function() {						
				if( typeof hash !== 'undefined' && hash.length > 0 ){				
					...
					setTimeout(function () {
						document.location.href = 'https://waardebonnen.consumentcentraal.nl?h=' + hash + '&oid=' + offer_id;
					}, 3000);
				}else{
					return origRedirect;	
				}			
		...
		if( $('#telnr-bevestiging').length > 0 ){
			...
			redirect = function() {						
				if( typeof hash !== 'undefined' && hash.length > 0 ){				
					...
					clearInterval(interval_bevestig);	
					setTimeout(function () {
						document.location.href = 'https://waardebonnen.consumentcentraal.be?h=' + hash + '&oid=' + offer_id;
					}, 3000);
				}else{
					return origRedirect;	
				}			
	...
	//prefill hack
	$('input, textarea').not(':input[type=button], :input[type=submit], :input[type=reset]').on("input", function ( e ) {		
		let id = e.target.id;		
		if( e.originalEvent.inputType === 'insertReplacementText' || typeof e.originalEvent.inputType === 'undefined' ){
			console.log( 'trigger' );
			$('#'+id).trigger('keypress');
		}
	});
	...
	$('#campaign_question').data( 'translate', $('#campaign_question').text().replace(/<(.|\n)*?>/g, '') );
	$('#campaign_answer').data( 'translate', $('#campaign_answer').text().replace(/<(.|\n)*?>/g, '') );
			...
			if(result !== 'Cannot fetch questions'){			
				$.each(result.data.questions, function( index, value ) {
					if( value.option_values !== null ) {
						...
						for( var i in obj ){
							$('#'+value.name).append($('<option>', { value : i }).text(obj[i]));
						}					
					}
				});
			}
	...
	$('#voornaam, #tussenvoegsel, #achternaam').blockInput({ regex: '[ÆæØøÅåßa-zA-Z ]' }); //    ÆæØøÅåß
