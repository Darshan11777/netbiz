                        ...
                        callback: function (value, validator, $field) {
							...
							if(post === true){
								if (!postcodeReg.test(postcode)){
									...
									if( huisnummer != "" ) {
										$.post( 'general-bin/postalcodechecker/postalcode.php', { type: 'nl_sixpp', postcode: postcode, huisnummer: huisnummer }).done(function( data ) {
                    ...
                    callback: {
                        callback: function (value, validator, $field) {
							...
							if( telefoonnummer.length == 10 ){
								...
								$.getJSON('general-bin/phonechecker/phonecheck_nl.php?phone='+telefoonnummer+'&check=uitgebreid&type=alles', function(data){
			...
			$.ajax({
				...
				success:function (data) {
					...
					if(data.conversion) {
                        ...
                        // Google ads
                        gtag('event', 'conversion', {'send_to': 'AW-793010008/0aPCCLmjwPQCENi-kfoC'});
									...
									success: function (result) {
										if( result.status === 'success' ){
											...
											if( pixel_src !== '' ){
												...
												var imagepixel = '<img height="1" width="1" style="display:none" src="'+pixel_src+'"/>';
												$(imagepixel).appendTo('.tracking');
											}else{
												var iframe = '<iframe src="https://sendtportal.com/partnerpixels/rendered/'+transaction_id+'.html" scrolling="no" frameborder="0" width="1" height="1"></iframe>';
						...
						// Event tracking
                        gtag('event', 'view_item', {'send_to': ga_property, 'event_label' : 'P'+prelander+'. S8. Conversie failed', 'event_category' : tag});
